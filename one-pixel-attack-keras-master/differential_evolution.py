...
#from scipy._lib import xrange, string_types
import warnings
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, args=(),
                 ...
                 init='latinhypercube', atol=0):
        # check if startegy is in binimoal list or exponential list // by kyle
        ...
        # bounds가 [(0,32), (0,32), (0, 256), (0, 256) (0,256)] 으로 들어왔다면
        # 0 0 0 0 0
        # 32 32 256 256 256 이런 2차원 배열로 만들어준다
        self.limits = np.array(bounds, dtype='float').T # DE 돌리기 편하게 바꿔주는건가..?
        ...
        if maxfun is None:  # the default used to be None # 연산을 최대 몇번정도 할지 정하는 upper bound
            maxfun = np.inf
        self.maxfun = maxfun
        ...
        self.__scale_arg1 = 0.5 * (self.limits[0] + self.limits[1]) # (16, 16, 128, 128, 128)
        self.__scale_arg2 = np.fabs(self.limits[0] - self.limits[1]) # fabs()는 절댓값을 float 형으로 (32, 32, 256, 256, 256)

        self.parameter_count = np.size(self.limits, 1) #한쪽 리스트 값 개수만 세도 파라미터 몇개인지 알 수 있다..?
        # 1픽셀이면 5, 3픽셀이면 15, ....
        ...
        # 여기서 parameter count를 곱할거라 미리 나눠줬다. 따라서 num_p_m = 400
        self.population_shape = (self.num_population_members,
                                 self.parameter_count)

        self._nfev = 0
        if isinstance(init, str): #string_types is original
    ...
    def init_population_lhs(self):
        ...
        # segsize는 element 하나당의 간격을 의미한다. population size = 400이고 parameter_count = 5 (1pixel)
        # 이면 2000일텐데 저렇게 짜른다는건 각 벡터의 하나의 element를 보고싶다는 뜻

        # Within each segment we sample from a uniform random distribution.
        # We need to do this sampling for each parameter.
        # population shape = (num_population_members, parameter_count)
        ...
        # 만들어진걸 population에 [0,1) 사이의 값을 uniform하게 넣는다
    ...
    def init_population_array(self, init):
        ...
        # 정규화 시키는게 unscale, 정규화 돤 걸 다시 원래대로 돌리는게 scale parameter
        # unscale parameter = return (parameters - self.__scale_arg1) / self.__scale_arg2 + 0.5
    ...
    def solve(self):
        ...
        status_message = 'success' #_status_message['success'] is original
        ...
        # do the optimisation.
        for nit in range(1, self.maxiter + 1): # original: range -> xrange
            # evolve the population by a generation
            try:
                next(self)
            except StopIteration:
                warning_flag = True
                status_message = 'maxfev' # _status_message['maxfev'] is original
                break
            ...
            status_message = 'maxiter' #_status_message['maxiter'] is original
            warning_flag = True
    ...
    def __next__(self):
        ...
        ## CHANGES: self.func operates on the entire parameters array
        ##############
        # mutate and crossover section
        itersize = max(0, min(self.num_population_members, self.maxfun - self._nfev + 1))
        trials = np.array([self._mutate(c) for c in range(itersize)]) # TODO: can be vectorized # mutate, crossover 하는 부분
        ...
        for candidate,(energy,trial) in enumerate(zip(energies, trials)):
            ...
            if energy < self.population_energies[candidate]:
                ...
                # 이 부분도 동일, 전까지 구한 베스트 솔루션보다 좋다면 대체시키는 부분인듯
    ...
    def _mutate(self, candidate):
        ...
        """
        # mutate가 점점 더 중심으로 가는 그림을 의미... crossover는 말 그대로 crossover
        trial = np.copy(self.population[candidate])

        rng = self.random_number_generator

        fill_point = rng.randint(0, self.parameter_count)
        # bprime안에 계산된 값이 들어감, 이 부분은 donor vector 만드는 부분
    ...
    def _best1(self, samples):
        ...
        best1bin, best1exp
        """
        # see enerygy by this mutation strategy
        r0, r1 = samples[:2]
